...
!defined('IN_TOA') && exit('Access Denied!');
 class upload_file
 {
  ...
  {
   $this->extention_list = "doc|docx|xlsx|jpg|gif|rar|txt|pdf|zip|xls|ppt|esp|cad|3ds|img|psd|pdf|ai|log|mdb|pot|csv|ceb"; //默认允许上传的扩展名称;
  } 
  ...
  function copy_file()
  {
   if(copy($this->upfile,$this->file_path)){        //上传文件;
	   ...
	   if($_GET['create_url']==''){
	   	   ...
	   	   echo "<script>window.opener.document.save.".$_GET[name].".value+='【<a href=".$this->file_path.">".$this->upfile_name."</a>】';window.opener.document.all.div_filelist.innerHTML+='".$this->upfile_name." <font onclick=OM_delete_fl(\"".$this->file_path."\") color=red>删除</font><br>'; </script>";
