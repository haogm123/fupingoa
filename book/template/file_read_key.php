...
<a href="admin.php?ac=file_read_key&fileurl=book&type=1" class="ui-tab-trigger-text">等待审批</a></li>
<li class="ui-tab-trigger-item <?php if($_GET[type]=='3' && $_GET[types]!='1'){?>ui-tab-trigger-item-current<? }?>">
<a href="admin.php?ac=file_read_key&fileurl=book&type=3" class="ui-tab-trigger-text">已审批</a></li>	
<li class="ui-tab-trigger-item <?php if($_GET[types]=='1'){?>ui-tab-trigger-item-current<? }?>">
<a href="admin.php?ac=file_read_key&fileurl=book&type=3&types=1" class="ui-tab-trigger-text">已借出</a></li>
<li class="ui-tab-trigger-item <?php if($_GET[type]==''){?>ui-tab-trigger-item-current<? }?>">
<a href="admin.php?ac=file_read_key&fileurl=book" class="ui-tab-trigger-text">图书一览</a></li>					
</ul>
				</div>
...
<td class="checkbox">
<?php
get_boxlistkey("id[]",$row['id'],$row['examperson'],$_USER->id)
?></td>
...
<?php
							global $db;
							$r = $db->fetch_one_array("SELECT b.* FROM ".DB_TABLEPRE."book a,".DB_TABLEPRE."book_type b where a.id='".$row['bookid']."' and a.booktype=b.id and b.keyuser like '%".get_realname($_USER->id)."%'  ORDER BY a.id desc limit 0,1");
							if ($r[keyuser]!=''){
							?>
							<?php if($row[type]=='1'){?>
							<a href="admin.php?ac=file_read_key&tid=<?php echo $row['id']?>&ltype=3&do=save&fileurl=book">审批</a> | <a href="admin.php?ac=file_read_key&do=save&fileurl=book&tid=<?php echo $row['id']?>&ltype=2">拒绝</a>
							<?php }elseif($row[type]=='3'){?>
							<a href="admin.php?ac=file_read_key&tid=<?php echo $row['id']?>&ltype=4&do=save&fileurl=book">归还图书</a>
							<?php }else{?>&nbsp;
							<?php echo "正常";}
							}else{
							echo "无权限";
