...
if ($do == 'list') {
	...
	if($_POST[save_type]!=2){        ////发布：1  草稿：2
	if($_POST['view']!=''){
		$id = getGP('id','P','int');
		if($id!=''){
			$categoryid = 0;
...
elseif ($do == 'views') {
	...
	//草稿箱信息 
	if($_GET[id]!=''){//编辑
		$user = $db->fetch_one_array("select * from toa_news_draft where id=$_GET[id]");
		include_once('template/newsadd.php');
	}else{//草稿列表
		$myid=$_USER->id;//登陆人ID
		$ttype=$_GET[type];//得到类型
		$sql="select id,subject, date from toa_news_draft where uid=$myid and type=$ttype order by date desc";
		//echo $sql;
		$result = $db->fetch_all($sql);
		include_once('template/draft.php');
	}
}elseif($do=='draft_delect'){
	$db->query("DELETE FROM ".DB_TABLEPRE."news_draft WHERE id = $_GET[id]");
