...
<form name="save" method="post" action="?ac=humanadd&do=addsave&fileurl=<?php echo $fileurl?>&type=<?php echo $type?>">
   ...
   <tr>
	 ...
	 <? }?>
      ...
      <td nowrap class="TableContent">证照类型：</td>
      <td class="TableData" >
        <?php echo get_human_select('toa_2_LICENSE_TYPE',get_human_form_value('toa_2_LICENSE_TYPE',2,'inputvaluenum'),get_human_db($blog['id'],"toa_2_LICENSE_TYPE"))?>
      </td> 
    </tr>
    <tr>
    	 <td nowrap class="TableContent">证照编号：</td>
      <td class="TableData">
        <input type="text" name="toa_2_LICENSE_NO" id="toa_2_LICENSE_NO" class="BigInput" value="<?php echo get_human_db($blog['id'],"toa_2_LICENSE_NO")?>">
      </td>
    	<td nowrap class="TableContent">证照名称：</td>
      <td class="TableData">
       <input type="text" name="toa_2_LICENSE_NAME" id="toa_2_LICENSE_NAME" class="BigInput" value="<?php echo get_human_db($blog['id'],"toa_2_LICENSE_NAME")?>">
      </td>
    </tr>
    <tr>
      <td nowrap class="TableContent">取证日期：</td>
      <td class="TableData">
      <?php echo get_human_date('toa_2_GET_LICENSE_DATE',get_human_db($blog['id'],"toa_2_GET_LICENSE_DATE"))?>
      </td>
      <td nowrap class="TableContent">生效日期：</td>
      <td class="TableData">
       <?php echo get_human_date('toa_2_EFFECTIVE_DATE',get_human_db($blog['id'],"toa_2_EFFECTIVE_DATE"))?>
      </td>
    </tr>
    <tr>
      <td nowrap class="TableContent">状态：</td>
      <td class="TableData">
        <?php echo get_human_select('toa_2_STATUS',get_human_form_value('toa_2_STATUS',2,'inputvaluenum'),get_human_db($blog['id'],"toa_2_STATUS"))?>
      </td>
      <td nowrap class="TableContent">到期日期：</td>
      <td class="TableData">
      	<?php echo get_human_date('toa_2_EXPIRATION_PERIOD',get_human_db($blog['id'],"toa_2_EXPIRATION_PERIOD"))?> 
      </td> 
    </tr>    
   <tr>
      <td nowrap class="TableContent">发证机构：</td>
      <td class="TableData" colspan=3>
        <textarea name="toa_2_NOTIFIED_BODY" cols="70" rows="3" class="BigInput" value=""><?php echo get_human_db($blog['id'],"toa_2_NOTIFIED_BODY")?></textarea>
      </td>
    </tr>
    <tr>
      <td nowrap class="TableContent">备注：</td>
      <td class="TableData" colspan=3>
        <textarea name="toa_2_REMARK" cols="70" rows="3" class="BigInput" value=""><?php echo get_human_db($blog['id'],"toa_2_REMARK")?></textarea>
      </td>
    </tr> 
