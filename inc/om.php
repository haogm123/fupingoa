...
<style type="text/css">
#OM_scfyj{font-size:12px;line-height:1.5em;color:#849BCA;background:#EDF1F8;border:2px solid #849BCA;overflow:hidden;position:absolute;display:none;width:0px;height:0px;filter:alpha(opacity=100);opacity:1;}
...
#OM_current_UL li,#OM_weidu_UL li{height:25px;width:100%;background:#eef;color:#CCf;cursor:pointer;font-weight:bold;padding-top:5px;}
...
<div id="OM_scfyj">
	...
	<div style="position:absolute;left:0px;top:300px;width:100%;height:160px;border-top:solid 1 #f90;float:left">
		...
		<div class="tit2"> &nbsp;<button id="OM_select_receiver" style="width:80px;height:25px;" onclick="window.open ('admin.php?ac=user_checkbox&fileurl=public&inputname=OM_receiver&user=', 'newwindow', 'height=500, width=500, top=50, left=100, toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no');return false;">选择接收人：</button>
			...
			<textarea id="OM_Send_Msg" name="OM_Send_Msg" onkeydown="send_Key_check()"></textarea>
			</form>
			<button id="OM_Send_Button" name="OM_Send_Button" onclick="send_click();return false;" style="width:60px;height:25px;">发送消息</button>
		</div>
		<div class="tit3">
			...
			<input type="radio" name="sendKey" onclick="OM_sendKey='';">按Enter发送<br><br>
		</form>
			<button onclick="window.open ('admin.php?ac=uploadadd&fileurl=public&name=OM_Send_Msg&create_url=1', 'newwindow', 'height=200, width=400, top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no')" style="width:60px;height:25px;">发送附件</button>	
		</div>
...
var OM_Send_Msg=document.getElementById('OM_Send_Msg');
//var OM_Send_Button=document.getElementById('OM_Send_Button');
OM_Timer1();
function OM_Timer1(){
			...
			if(OM_CtrlStr.indexOf("[Send_OK]")>-1 && str=="send_msg"){//如果操作中包含发送消息且服务器返回成功信息
				...
				for (i=0;i<str_arr.length;i++)
				{
					...
					if(e){
						...
						d.id="OM_Msg_LIST2";
						d.innerHTML="<p><font color='#337733'>"+server_Time+"</font> <font color='red'>[ 我 ] </font><br>"+nl2br(msg)+"</p>";
...
//-----------------------------------------------------------------------------
				...
				for(i=1;i<str_arr.length;i+=3){
					...
					if(e){
						...
						d=document.createElement("pre");
						d.id="OM_Msg_LIST1";
						d.innerHTML="<p> <font color='blue'> "+document.getElementById("OM_session_LI_n"+str_arr[i]).innerHTML+" </font> <font color='#F90'>"+str_arr[i+1]+"</font><br>"+nl2br(str_arr[i+2])+"</p>";
						object=e.appendChild(d);
						e.scrollTop=d.offsetTop;
...
//-----------------------------------------------------------------------------------
		...
		}else if(getStr==''){//如果服务器返回空
			if (str!='auto_run'){alertt("网络超时或服务器无响应，操作未成功！");}
