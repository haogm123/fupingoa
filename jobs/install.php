...
if($step=='2'){
	...
	if($dbbackup->import("mysql.sql")){				//导入数据
		show_msg('招聘管理组件模块安装成功！', '../admin.php?ac=plugin&fileurl=mana');
	}
}elseif($step=='1'){
	global $db;
	...
	//数据库
	$db->query("DROP TABLE toa_job");
	$db->query("DROP TABLE toa_job_demand");
	$db->query("DROP TABLE toa_job_hire");
	$db->query("DROP TABLE toa_job_talent");
	show_msg('招聘管理组件模块卸载成功！', '../admin.php?ac=plugin&fileurl=mana');
}else{
	global $db;
...
<table width="60%" border="0" align="center" cellpadding="3" cellspacing="0" class="small" style="margin-top:20px;margin-bottom:10px;">
  <tr>
    <td class="Big"><img src="../template/default/content/images/notify_new.gif" align="absmiddle"><span class="big3"> 招聘管理集成组件安装</span>&nbsp;&nbsp;&nbsp;&nbsp;
