...
<div id="content" class="ui-content fn-clear" coor="default" coor-rate="0.02">
	<div class="ui-grid-21" coor="content">
		<div class="ui-grid-21 ui-grid-right record-tit" coor="title">
			<h2 class="ui-tit-page">人员录用信息列表</h2>
			
			<div class="record-tit-amount">
				<p>总共有<span class="number"><?php echo public_number('job_hire')?></span>条数据
              </p>
			</div>
		</div>
		...
		<div class="ui-grid-21 ui-grid-right fn-clear" coor="total">
		                                <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
                                          <tr>
                                            ...
                                            <td width="9%" align="right" style="padding-right:10px;"><input type="button" value="新增人员" class="BigButtonBHover" onClick="javascript:window.location='admin.php?ac=job_hire_add&fileurl=jobs'"></td>
                                          </tr>
                                        </table>
...
<a href="admin.php?ac=<?php echo $ac?>&fileurl=<?php echo $fileurl?>" class="ui-tab-trigger-text">人员录用信息列表</a></li>			
</ul>
				</div>
...
<div class="fn-left"><?php echo get_usershow('-1','admin.php?ac='.$ac.'&fileurl='.$fileurl.'&ischeck='.$ischeck.'&job='.$job.'&department='.$department.'&jobpost='.$jobpost.'')?></div>
<div class="fn-right">
<?php echo get_exceldown('excel_29')?></div>
</div>
						...
						<table id="tradeRecordsIndex" class="ui-table">
							...
							<thead>
								<tr>
									...
									<th width="100">被录用人</th>
									<th class="title">招聘计划</th>
									<th width="80">录用负责人</th>
									<th width="120">用人部门</th>
									<th width="100">录用日期</th>
									<th width="80">录用岗位</th>
									<th class="info">备注</th>
									<th width="80">发布人</th>
...
<td width="100"><?php echo get_talent_name($row['job_talent'])?></td>
<td class="title"><?php echo get_job_name($row['jobpost'])?>
</td>
<td width="80"><?php echo $row['user']?></td>
<td width="120"><?php echo $row['department']?></td>
<td width="100"><?php echo trim($row['hiredate'])?></td>
<td width="80"><?php echo get_postname($row['job'])?></td>
<td class="info"><?php echo $row['content']?></td>
<td width="80"><?php echo get_realname($row['uid'])?></td>
<td class="action">
<?php get_urlkey("编辑","admin.php?ac=job_hire_edit&fileurl=jobs&do=edit&id=".$row['id']."","".$row['uid']!=$_USER->id)?>
</td>
