...
if ($do == 'list') {
	...
	$id = getGP('id','G','int');
	//foreach ($idarr as $id) {
	$db->query("DELETE FROM ".DB_TABLEPRE."file_type WHERE id = '$id'  ");
	//db->query("DELETE FROM ".DB_TABLEPRE."user_view WHERE uid = '$id'");
	$content=$id;
	$title='删除档案类别';
	get_logadd($id,$content,$title,20,$_USER->id);
	//}
	show_msg('删除档案类别成功！', 'admin.php?ac=file_type&fileurl=file&message=1');
...
function get_position_list_save($fatherid=0,$selid=0,$layer=0,$ac,$fileurl){
	...
	$sql="SELECT * FROM ".DB_TABLEPRE."file_type where father='$fatherid'   ORDER BY id Asc";
	$query = $db->query($sql);
	echo '<tbody id="group_'.trim($fatherid).'">';
	if(count($query)>0){
		while ($row = $db->fetch_array($query)) {
			$rsfno = $db->fetch_one_array("SELECT * FROM ".DB_TABLEPRE."file_type where father='".$row[id]."'  ORDER BY id asc limit 0,1");
			echo '<tr class="hover">';
			echo '<td class="td25"></td>';
			echo '<td class="td25">'.get_realname($row['uid']).'';
			echo '<td><div class="board">'.$row['title'].'</div></td>';
			echo '<td width="160">';
			get_urlkey("编辑","admin.php?ac=file_type_edit&do=edit&fileurl=file&id=".$row['id']."","".$row['uid']!=$_USER->id);
			
			if($rsfno[id]==''){
				echo '&nbsp;|&nbsp;';
				get_urlkey("删除","admin.php?ac=file_type&fileurl=file&do=delete&id=".$row['id']."","".$row['uid']!=$_USER->id);
			}
