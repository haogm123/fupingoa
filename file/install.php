...
if($step=='2'){
	...
	if($dbbackup->import("mysql.sql")){				//导入数据
		show_msg('档案管理组件模块安装成功！', '../admin.php?ac=plugin&fileurl=mana');
	}
}elseif($step=='1'){
	global $db;
	$db->query("UPDATE ".DB_TABLEPRE."plugin set type='1' WHERE id = '".$_POST['pid']."' ");
	//权限
	$db->query("DELETE FROM ".DB_TABLEPRE."keytable WHERE id = 364");
	$db->query("DELETE FROM ".DB_TABLEPRE."keytable WHERE id = 403");
	$db->query("DELETE FROM ".DB_TABLEPRE."keytable WHERE id = 404");
	for($i=84;$i<=102;$i++){
	...
	$db->query("DELETE FROM ".DB_TABLEPRE."menu WHERE menuid = 6");
	//数据库
	$db->query("DROP TABLE toa_file_read");
	$db->query("DROP TABLE toa_file_type");
	$db->query("DROP TABLE toa_file");
	show_msg('档案管理组件模块卸载成功！', '../admin.php?ac=plugin&fileurl=mana');
}else{
	global $db;
...
<table width="60%" border="0" align="center" cellpadding="3" cellspacing="0" class="small" style="margin-top:20px;margin-bottom:10px;">
  <tr>
    <td class="Big"><img src="../template/default/content/images/notify_new.gif" align="absmiddle"><span class="big3"> 档案管理集成组件安装</span>&nbsp;&nbsp;&nbsp;&nbsp;
