...
<table width="90%" border="0" align="center" cellpadding="3" cellspacing="0" class="small">
  <tr>
    <td class="Big"><img src="template/default/content/images/notify_new.gif" align="absmiddle"><span class="big3"> 档案信息编辑</span>&nbsp;&nbsp;&nbsp;&nbsp;
...
<form name="save" method="post" action="?ac=edit&do=save&fileurl=file">
...
<table class="TableBlock" border="0" width="90%" align="center" style="border-bottom:#4686c6 solid 0px;">
    ...
    <tr>
      ...
      <td class="TableData"><select class="BigStatic" name="filetype">
		<?php get_file_type_option(0,$blog['filetype'])?>
		</select></td>
      <td class="TableContent" width="120">档案编号：<? get_helps()?></td>
      <td class="TableData">
      <input name="filenumber" type="text" class="BigInput" id="filenumber" style="width: 300px;" maxlength="100" value="<?php echo $blog['filenumber']?>" /></td>
    </tr>
    <tr>
      <td nowrap class="TableContent" width="120"> 档案名称：<? get_helps()?></td>
      <td class="TableData"><input name="filename" type="text" class="BigInput" id="filename" style="width: 300px;" maxlength="100" value="<?php echo $blog['filename']?>" /></td>
      <td class="TableContent" width="120">保存期限：</td>
      <td class="TableData">
     <select name="enddate" class="BigStatic" id="enddate">
									  <option value="0">请选择</option>
									  <option value="5" <?php if($blog['enddate']=='5'){?>selected="selected"<?php }?>>五年</option>
									  <option value="99" <?php if($blog['enddate']=='99'){?>selected="selected"<?php }?>>永久</option>
									  <option  value="1" <?php if($blog['enddate']=='1'){?>selected="selected"<?php }?>>一年</option>
									  <option value="3" <?php if($blog['enddate']=='3'){?>selected="selected"<?php }?>>三年</option>
									  </select> </td>
    </tr>
	<tr>
      <td nowrap class="TableContent">存放位置：<? get_helps()?></td>
      <td class="TableData"><input name="position" type="text" class="BigInput" id="position" style="width: 300px;" onFocus="this.select();" maxlength="100" value="<?php echo $blog['position']?>" /> </td>
      <td class="TableContent">页数：</td>
      <td class="TableData"><input name="page" type="text" class="BigInput" id="page" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" value="<?php echo $blog['page']?>" />      </td>
    </tr>
    ...
    <tr>
      ...
      <td colspan="3" class="TableData">
	    <?php echo public_upload('file1',$appendix[0])?>	  </td>
    </tr>
	 <tr>
      <td nowrap class="TableContent">附件二：</td>
      <td colspan="3" class="TableData">
	    <?php echo public_upload('file2',$appendix[1])?>	  </td>
    </tr>
	 <tr>
      <td nowrap class="TableContent">附件三：</td>
      <td colspan="3" class="TableData">
	    <?php echo public_upload('file3',$appendix[2])?>	 </td>
    </tr>
