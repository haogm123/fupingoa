	...
	<div class="ui-grid-21" coor="content">
		<div class="ui-grid-21 ui-grid-right record-tit" coor="title">
			...
			<div class="record-tit-amount">
				<p>总共有<span class="number"><?php echo public_number('file')?></span>条数据
              </p>
			</div>
		</div>
		...
		<?php get_file_type_option(0,$filetype)?>
		</select>
					  </div>
...
<a href="admin.php?ac=indexsearch&fileurl=file" class="ui-tab-trigger-text">档案借阅</a></li>
<li class="ui-tab-trigger-item">
<a href="admin.php?ac=file_read&fileurl=file&type=1" class="ui-tab-trigger-text">等待借阅</a></li>	
<li class="ui-tab-trigger-item">
<a href="admin.php?ac=file_read&fileurl=file&type=3" class="ui-tab-trigger-text">已借档案</a></li>
<li class="ui-tab-trigger-item">
...
<div class="fn-left"><?php echo get_usershow('-1','admin.php?ac='.$ac.'&fileurl='.$fileurl.'&ischeck='.$ischeck.'&number='.$number.'&title='.$title.'&vstartdate='.$vstartdate.'&venddate='.$venddate.'&filetype='.$filetype.'&enddate='.$enddate.'')?></div>
<div class="fn-right">
<?php echo get_exceldown('excel_35')?></div>
</div>
...
<td class="title">
<a href="admin.php?ac=views&do=edit&fileurl=file&id=<?php echo $row['id']?>"><?php echo $row['filename']?></a>
</td>
<td width="100"><?php echo get_file_type_name($row['filetype'])?></td>
<td width="120"><?php
							if($row['enddate']=='1'){
							echo "一年";
							}elseif($row['enddate']=='3'){
							echo "三年";
							}elseif($row['enddate']=='5'){
							echo "五年";
							}elseif($row['enddate']=='99'){
							echo "永久";
							}
...
<?php if($row[type]!='1' && $row[type]!='3'){?>
							<a href="admin.php?ac=indexsearch&do=save&fileurl=file&id=<?php echo $row['id']?>">借阅</a>
							<?php 
							}else{
							echo "<font color=red>借阅中</font>";
							}
							?></td>						
</tr>
<?}?>			
