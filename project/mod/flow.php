...
$sql = "SELECT * FROM ".DB_TABLEPRE."project_flow  WHERE fid = '".$row['flowid']."'";
...
?>
	...
	<tr>
	  ...
	  <b>状态：</b><?php echo project_pertype($row['pertype'])?><br>
	...
	<?php }else{?>
		...
		<?php }else{
			$query = $db->query("SELECT * FROM ".DB_TABLEPRE."project_personnel_log where perid='".$row['perid']."' ORDER BY lid Asc");
