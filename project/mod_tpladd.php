...
if ($do == 'list') {
		if($tplid==''){
			include_once('tpllist/typelist.php');
			exit;
		}
		//生成流水号
		...
		if($mid['key2']==1){
			//读取流程数据
			$flow = $db->fetch_one_array("SELECT * FROM ".DB_TABLEPRE."project_flow  WHERE typeid = '".$typeid."' and tplid='".$tplid."' and flowkey4=3 and flownum>1 order by fid asc");
			if($flow['fid']==''){
				show_msg('无流程数据，不可用！', 'admin.php?ac=tpllist&fileurl='.$fileurl.'&modid='.$modid.'&projectid='.$projectid.'&typeid='.$typeid.'&tplid='.$tplid.'');
			}
		}
		...
		if($tpl['tplkey']==3){
			...
			include_once('tpllist/workadd.php');
		}
	...
	//写入单项数据
	global $db;
	$query = $db->query("SELECT * FROM ".DB_TABLEPRE."project_from WHERE typeid='".$typeid."' and formtype='3' and tplid='".$tplid."' and inputtype!=6 order by fromid asc");
	...
	if($mid['key2']==1){
		//处理流程
		$sql = "SELECT * FROM ".DB_TABLEPRE."project_flow  WHERE typeid = '".$typeid."' and flownum=1 and flowkey4=3 and tplid='".$tplid."' order by fid asc";
		...
		if(getGP('sms_info_box_work','P')!=''){
			$content='您有一个新"'.$title.'"需要审批,请点击查看!<a href="admin.php?ac=tpllist&fileurl='.$fileurl.'&type=1&modid='.$modid.'&projectid='.$projectid.'&typeid='.$typeid.'&tplid='.$tplid.'">点击审批>></a>';
		...
		//通知其它人员
		if(getGP('viewuser','P')!=''){
			$content2='您接收到一条项目'.$mid['modname'].'为"'.$title.'"的通知,请点击“<a href="admin.php?ac=tpllist&do=view&fileurl='.$fileurl.'&lid='.$id.'&modid='.$modid.'&projectid='.$projectid.'&typeid='.$typeid.'&tplid='.$tplid.'">查看</a>”!';
	...
	$title='新建'.$mid['modname'].'数据';
	get_logadd($id,$content,$title,32,$_USER->id);
	show_msg($title.' 信息提交成功！', 'admin.php?ac=tpllist&fileurl='.$fileurl.'&modid='.$modid.'&projectid='.$projectid.'&typeid='.$typeid.'&tplid='.$tplid.'');
}
?>