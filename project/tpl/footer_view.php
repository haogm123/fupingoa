...
<?php
if($row['tplid']!=''){
	$sql = "SELECT * FROM ".DB_TABLEPRE."project_personnel where projectid='".$row['lid']."' and appkey2=3 order by perid asc";
}else{
...
?>
	...
	<?php }else{?>
		...
		<?php }else{
			$query = $db->query("SELECT * FROM ".DB_TABLEPRE."project_personnel_log where perid='".$rows['perid']."' and appkey2=2 ORDER BY lid Asc");
