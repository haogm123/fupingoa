...
<div id="navPanel">
<div id="navMenu">
<a href="admin.php?ac=<?php echo $ac?>&do=view&fileurl=<?php echo $fileurl?>&modid=<?php echo $modid?>&projectid=<?php echo $projectid?>&typeid=<?php echo $typeid?>&tplid=<?php echo $tplid?>&lid=<?php echo $row['lid']?>&down=ture"><span>
...
<table width="80%" border="0" align="center" cellpadding="3" cellspacing="0" class="small">
  <tr>
	...
	<a href="admin.php?ac=<?php echo $ac?>&fileurl=<?php echo $fileurl?>&modid=<?php echo $modid?>&projectid=<?php echo $projectid?>&typeid=<?php echo $typeid?>&type=<?php echo $_GET['type']?>&tplid=<?php echo $tplid?>" style="font-size:12px;"><< 返回<?php echo $mod['modname']?>列表</a>
...
<?php
global $db;
$query = $db->query("SELECT a.*,b.inputtype,b.fromname FROM ".DB_TABLEPRE."project_db a,".DB_TABLEPRE."project_from b where a.projectid='".$row['lid']."' and a.fromid=b.fromid and b.inputtype!='6'  and a.typeid='".$typeid."' and a.tplid='".$tplid."' and (a.type='3' or a.type='4') and a.modid='".$modid."' ORDER BY a.did Asc");
	...
	//处理多输入表单
	global $db;
	$nums = $db->result("SELECT a.*,b.inputtype,b.fromname FROM ".DB_TABLEPRE."project_db a,".DB_TABLEPRE."project_from b where a.projectid='".$row['lid']."' and a.fromid=b.fromid and b.inputtype='6'  and a.typeid='".$typeid."' and a.tplid='".$tplid."' and a.type='3' and a.modid='".$modid."' ORDER BY a.did Asc");
	$sql = "SELECT a.*,b.inputtype,b.fromname FROM ".DB_TABLEPRE."project_db a,".DB_TABLEPRE."project_from b where a.projectid='".$row['lid']."' and a.fromid=b.fromid and b.inputtype='6'  and a.typeid='".$typeid."' and a.type='3' and a.tplid='".$tplid."' and a.modid='".$modid."' ORDER BY a.did Asc";
...
<?php if($mod['key1']==1){?>
<form name="save" method="post" action="admin.php?ac=<?php echo $ac?>&do=view&fileurl=<?php echo $fileurl?>&lid=<?php echo $row['lid']?>&modid=<?php echo $modid?>&projectid=<?php echo $projectid?>&typeid=<?php echo $typeid?>&tplid=<?php echo $tplid?>&type=<?php echo $_GET['type']?>" style="padding:0px; margin:0px;">
...
<?php
$sql = "SELECT * FROM ".DB_TABLEPRE."project_personnel where projectid='".$row['lid']."'  order by perid asc";
