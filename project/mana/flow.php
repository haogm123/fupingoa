...
<table width="90%" border="0" align="center" cellpadding="3" cellspacing="0" class="small">
  <tr>
	...
	?>
	<a href="<?php echo $acs?>" style="font-size:12px;"><<返回上级列表</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="新建流程" class="BigButtonBHover" onClick="javascript:window.location='admin.php?ac=<?php echo $ac?>&fileurl=<?php echo $fileurl?>&do=add&tplid=<?php echo $tplid?>&typeid=<?php echo $typeid?>&flowkey4=<?php echo $flowkey4?>'">
	</span>
    </td>
  </tr>
</table>

<form name="update" method="post" action="admin.php?ac=<?php echo $ac?>&fileurl=<?php echo $fileurl?>&tplid=<?php echo $tplid?>&typeid=<?php echo $typeid?>&flowkey4=<?php echo $flowkey4?>">
...
<?foreach ($result as $row) {?>
	<tr>
	  ...
	  <?php
	  $anum=$db->result("SELECT COUNT(*) AS anum FROM ".DB_TABLEPRE."project_personnel where flowid='".$row['fid']."'");
	  ?>
	  ...
	  <a href="admin.php?ac=<?php echo $ac?>&do=add&fileurl=<?php echo $fileurl?>&tplid=<?php echo $tplid?>&typeid=<?php echo $typeid?>&fid=<?php echo $row['fid']?>&flowkey4=<?php echo $flowkey4?>&modid=<?php echo $modid?>">编辑</a> | 
	  <a href="admin.php?ac=form&fileurl=<?php echo $fileurl?>&tplid=<?php echo $tplid?>&typeid=<?php echo $typeid?>&modid=<?php echo $modid?>&formtype=4&flowid=<?php echo $row['fid']?>&formtypes=<?php echo $_GET['flowkey4']?>">表单管理</a>
	  <?php }?>
	  </td>
    </tr>
