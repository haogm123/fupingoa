...
<form method="get" action="admin.php" name="topSearchForm" class="ui-grid-21 ui-grid-right ui-form" style=" margin-top:3px;">
	...
	名称：<input type="text" name="title" size="15"
...
<table width="90%" border="0" align="center" cellpadding="3" cellspacing="0" class="small">
  <tr>
    <td class="Big"><img src="template/default/content/images/notify_new.gif" align="absmiddle"><span class="big3">项目模板管理</span>
	<span style="font-size:12px; float:right; margin-right:20px;"><a href="admin.php?ac=model&fileurl=<?php echo $fileurl?>&typeid=<?php echo $typeid?>" style="font-size:12px;"><<返回上一级</a>&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="button" value="新增模板" class="BigButtonBHover" onClick="javascript:window.location='admin.php?ac=<?php echo $ac?>&do=add&fileurl=<?php echo $fileurl?>&typeid=<?php echo $typeid?>&modid=<?php echo $modid?>'">
	</span>
    </td>
  </tr>
</table>
<form name="update" method="post" action="admin.php?ac=<?php echo $ac?>&fileurl=<?php echo $fileurl?>&typeid=<?php echo $typeid?>&modid=<?php echo $modid?>">
		<input type="hidden" name="do" value="update" />	 
	 <table class="TableBlock" border="0" width="90%" align="center">
		<tr>
			<td nowrap class="TableHeader" width="30"></td>
			  <td width="120" align="center" class="TableHeader">流程类别</td>
			  <td class="TableHeader">模板名称</td>
	...
	<?php foreach ($result as $row) { ?>
	  ...
	  <?php
	 if(!is_superadmin()){
		 $dnum=$db->result("SELECT COUNT(*) AS dnum FROM ".DB_TABLEPRE."project_from where tplid='".$row['tplid']."' and formtype=3");
		if($dnum<1){
			if($row['tpltype']!='1'){
	...
	?>
      ...
      <td align="center" class="TableData"><?php echo public_value('title','project_type'," tid='".$row['typeid']."'")?></td>
      <td class="TableData">
	  <?php
	  if(file_exists("project/tpl/".$row['tplid'].'_'.$row["tpladdr"])!=''){
    ...
    <td align="center" class="TableData">
	  <?php get_urlkey("编辑","admin.php?ac=".$ac."&do=edit&fileurl=".$fileurl."&tplid=".$row['tplid']."&typeid=".$row['typeid']."&modid=".$row['modid']."","".$row['uid']!=$_USER->id)?> | 
<a href="admin.php?ac=form&fileurl=<?php echo $fileurl?>&tplid=<?php echo $row['tplid']?>&formtype=3&typeid=<?php echo $row['typeid']?>&modid=<?php echo $row['modid']?>">表单管理</a> | 
<a href="admin.php?ac=flow&fileurl=<?php echo $fileurl?>&tplid=<?php echo $row['tplid']?>&typeid=<?php echo $row['typeid']?>&flowkey4=3&modid=<?php echo $row['modid']?>">流程设置</a>
	  </td>
    </tr>
	<?php } ?>
