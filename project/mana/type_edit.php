...
<table width="70%" border="0" align="center" cellpadding="3" cellspacing="0" class="small">
  <tr>
    <td class="Big"><img src="template/default/content/images/notify_new.gif" align="absmiddle"><span class="big3"> 项目类型编辑</span>&nbsp;&nbsp;&nbsp;&nbsp;
...
<form name="save" method="post" action="?ac=<?php echo $ac?>&do=editsave&fileurl=<?php echo $fileurl?>">
  <input type="hidden" name="tid" value="<?php echo $view['tid'];?>" />
<table class="TableBlock" border="0" width="70%" align="center">

	<tr>
      <td nowrap class="TableContent" width="15%"> 类型名称：<? get_helps()?></td>
      <td class="TableData">
      <input type="text" name="title" class="BigInput" style="width:268px;" size="20" value="<?php echo $view['title'];?>" /></td>
    </tr>
	<tr>
      <td nowrap class="TableContent" width="15%"> 状态：<? get_helps()?></td>
      <td class="TableData">
	  <input name="type1" type="radio" value="0" <?php if($view['type1']==0){?>checked="checked"<?php }?> />正常
      <input name="type1" type="radio" value="1" <?php if($view['type1']==1){?>checked="checked"<?php }?> />停用
	  </td>
    </tr>
	<tr>
