...
function filenumber_show()
{
   jQuery.ajax({
      type: 'GET',
      url: 'admin.php?ac=file&fileurl=public&filenumber=<?php echo $filenumber?>&officetype=5&'+new Date(),
...
<form name="save" method="post" action="?ac=<?php echo $ac?>&do=add&fileurl=<?php echo $fileurl?>&typeid=<?php echo $typeid?>" style="margin-top:5px; margin-left:0px; margin-right:0px;">
...
<div id="search" style="float: left; padding-left:30px;">
	...
	项目名称<? get_helps()?>：<input type="text" name="title" class="BigInput" style="width:300px;" size="20" value="流水号[<?php echo $number?>]项目" />&nbsp;&nbsp;&nbsp;&nbsp;
	项目绑定： <input name="numberview" readonly type="text" class="BigInput" id="numberview" style="width: 120px;" value="" /><input name="numberurl" readonly type="hidden" id="numberurl"/><a href="#" onClick="window.open ('admin.php?ac=project&fileurl=public&protype=mana&number=<?php echo get_date('YmdHis',PHP_TIME)?>', 'newwindow', 'height=500, width=500, top=50, left=100, toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no')">+选择流水号</a>&nbsp;&nbsp;
...
<tr>
	...
	<!--<tr>
      <td nowrap class="TableContent" width="15%">项目周期：</td>
	  <td class="TableData">从 <input type="text" name="statdate" style="width:120px; height:20px;" value="<?php echo get_date('Y-m-d',PHP_TIME);?>" onClick="WdatePicker();"/> 到 <input type="text" name="enddate" style="width:120px; height:20px;" value="<?php echo get_date('Y-m-d',PHP_TIME);?>" onClick="WdatePicker();"/> 结束</td>
	  </tr>  -->
</table> 
<?
global $db;
...
//处理多输入表单
global $db;
$num = $db->result("SELECT * FROM ".DB_TABLEPRE."project_from where typeid='".$typeid."'  and formtype=1 and inputtype=6 ORDER BY fromid Asc");
$sql = "SELECT * FROM ".DB_TABLEPRE."project_from where typeid='".$typeid."'  and formtype=1 and inputtype=6 ORDER BY fromid Asc";
...
<tr>
	...
	<tr>
	  ...
	  <a href="#m2" onClick="window.open ('admin.php?ac=uploadadd&fileurl=public&name=annexurlid&filenumber=<?php echo $filenumber?>&officetype=5', 'newwindow', 'height=200, width=400, top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=no,location=no, status=no')">+上传附件</a></td>
    </tr>
</table>
<?php if($flow['fid']!=''){?>
<table class="TableBlock" border="0" width="80%" align="center">
