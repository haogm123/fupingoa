...
if ($do == 'list') {
	...
	if($_GET['type']==1){
		$num = $db->result("SELECT COUNT(*) AS num FROM ".DB_TABLEPRE."project_log a,".DB_TABLEPRE."project_personnel b WHERE 1 $wheresqltype and  a.lid=b.projectid and (b.pertype=0 or b.pertype=4) and b.name like '%".get_realname($_USER->id)."%' and a.type!=1 and b.appkey2=2 order by b.perid asc");
		$sql = "SELECT a.* FROM ".DB_TABLEPRE."project_log a,".DB_TABLEPRE."project_personnel b WHERE 1 $wheresqltype and  a.lid=b.projectid and (b.pertype=0 or b.pertype=4) and b.name like '%".get_realname($_USER->id)."%' and a.type!=1 and b.appkey2=2 order by b.perid asc LIMIT $offset, $pagesize";
	}elseif($_GET['type']==3){
	...
	if($_POST['view']!=''){
		...
		if(getGP('flowfrom','P')=='1'){//fromflowid
			$sql = "SELECT typeid FROM ".DB_TABLEPRE."project_log  WHERE lid = '".$lid."'";
		...
		include_once('mod/personnel.php');
	}
}elseif ($do == 'flow') {
	$sql = "SELECT * FROM ".DB_TABLEPRE."project_personnel where projectid='".$_GET['lid']."' and appkey2=2 order by perid asc";
	$result = $db->fetch_all($sql);
	include_once('mod/flow.php');
}elseif ($do == 'workkey') {
	$db->query("update ".DB_TABLEPRE."project_log set type=1 where lid = '".getGP('lid','G')."'");
	$content=getGP('projectid','G');
	$title='撤消项目'.$mod['modname'];
	get_logadd($id,$content,$title,32,$_USER->id);
	show_msg('项目己成功撤消！', 'admin.php?ac='.$ac.'&fileurl='.$fileurl.'&modid='.$modid.'&projectid='.$projectid.'&typeid='.$typeid.'');
	...
	if($_POST['view']=='edit'){
		...
		get_logadd($id,$content,$title,32,$_USER->id);
		show_msg('信息回复成功！', 'admin.php?ac='.$ac.'&fileurl='.$fileurl.'&type='.$_GET['type'].'&modid='.$modid.'&projectid='.$projectid.'&do=view&typeid='.$typeid.'&lid='.$_GET['lid']);
	}else{
		$sql = "SELECT * FROM ".DB_TABLEPRE."project_log  WHERE lid = '".$_GET['lid']."'";
		$row = $db->fetch_one_array($sql);
		if($mod['key4']==3){
			include_once('tpl/'.$typeid.'_'.$modid.'_view.php');
		}else{
			if($_GET['down']!=''){
				if(file_exists("project/tpl/".$typeid."_".$modid.".php")!=''){
					include_once("project/tpl/".$typeid."_".$modid.".php");
				}else{
					include_once('mod/view_excel.php');
				}
	...
	foreach ($idarr as $id) {
		...
		$db->query("DELETE FROM ".DB_TABLEPRE."project_db WHERE projectid = '$id' and type=2");
		$db->query("DELETE FROM ".DB_TABLEPRE."fileoffice WHERE officetype=6 and fileid=0 and officeid = '$id' ");	
	}
	$content=serialize($idarr);
	$title='删除项目'.$mod['modname'];
	get_logadd($id,$content,$title,32,$_USER->id);
	show_msg('项目删除成功！', 'admin.php?ac='.$ac.'&fileurl='.$fileurl.'&type='.$_GET['type'].'&modid='.$modid.'&projectid='.$projectid.'&typeid='.$typeid.'');
	...
	$outputFileName = 'data/excel/'.$datename.'.xls';
		...
		//获取表单
			...
			while ($row = $db->fetch_array($result)) {	
				...
				$archive[]=$row['title'];
				for($i=0;$i<$num;$i++){
					$blog = $db->fetch_one_array("SELECT * FROM ".DB_TABLEPRE."project_db  WHERE projectid = '".$row['lid']."' and inputname='".$inputname[$i]."' and type=2 ");
