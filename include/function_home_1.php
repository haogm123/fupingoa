...
function home_app($num){
	...
	$query = $db->query("SELECT type,fileid FROM ".DB_TABLEPRE."personnel where (pkey=0) and name like '%".get_realname($_USER->id)."%' order by type asc");
	while ($row = $db->fetch_array($query)) {
		if($row['type']==1){
			$table = "attachment";
			$sql = "SELECT category,title,id,type FROM ".DB_TABLEPRE.$table."  WHERE id = '".$row['fileid']."'";
		...
		if($rs['type']=='1')
			{
			echo '】<font color=red>【急件】</font><a href="admin.php?ac='.$table.'&do=view&fileurl=app&fileid='.$rs['id'].'&apptype=';
			}else{
