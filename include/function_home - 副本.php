...
function home_app($num){
	...
	//20141203  change
	//$query = $db->query("SELECT a.type,a.fileid FROM ".DB_TABLEPRE."personnel a,".DB_TABLEPRE."personnel_log b where (a.pkey=0 and a.name like '%".get_realname($_USER->id)."%' and a.appkey=2) or ((a.pkey=0 or a.pkey=4) and a.appkey=1 and a.id=b.perid and b.pkey=0 and b.uid=$_USER->id) group by a.fileid order by a.fileid asc");
	$query = $db->query("select c.type,c.fileid from(select a.type,a.fileid,(case when a.pkey=0 then 0 when a.appkey=1 then (select b.pkey from ".DB_TABLEPRE."personnel_log b where b.uid=$_USER->id and b.perid=a.id) end) as showd from ".DB_TABLEPRE."personnel a where a.name like '%".get_realname($_USER->id)."%' and a.pkey in(0,4)) c where c.showd=0 order by c.fileid");
	...
	//http://127.0.0.1/admin.php?ac=attachment&do=personnel&fileurl=app&fileid=216&apptype=18
