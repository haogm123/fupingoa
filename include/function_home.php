...
function home_app($num){
	...
	echo '<div id="module_'.$num.'_body" class="module_body">';
	echo '<div id="module_'.$num.'_ul" class="module_div">';
//echo "(select c.type,c.fileid,(case 1 when 1 then 0 end)as did from(select a.type,a.fileid,(case when a.pkey=0 then 0 when a.appkey=1 then (select b.pkey from ".DB_TABLEPRE."personnel_log b where b.uid=$_USER->id and b.perid=a.id) end) as showd from ".DB_TABLEPRE."personnel a where a.name like '%".get_realname($_USER->id)."%' and a.pkey in(0,4)) c where c.showd=0 order by c.fileid asc) union all (select dkey as 'type',fileid,did from toa_distribution where isnull(viewdate) and uid='".$_USER->id)."')";
	//echo "<h1>select c.type,c.fileid from(select a.type,a.fileid,(case when a.pkey=0 then 0 when a.appkey=1 then (select b.pkey from ".DB_TABLEPRE."personnel_log b where b.uid=$_USER->id and b.perid=a.id) end) as showd from ".DB_TABLEPRE."personnel a where a.name like '%".get_realname($_USER->id)."%' and a.pkey in(0,4)) c where c.showd=0 order by c.fileid asc</h1>";
	//已审批的不在显示器
	//$query = $db->query("SELECT type,fileid FROM ".DB_TABLEPRE."personnel where (pkey=0 or pkey=4) and name like '%".get_realname($_USER->id)."%' order by id asc");
		...
		$tmp_perid_str.='0';
	$query = $db->query("(select a.type,a.fileid,'0' as did from ".DB_TABLEPRE."personnel a where a.name like '%".get_realname($_USER->id)."%' and a.pkey in(0,4) and (a.pkey=0 or a.id in(".$tmp_perid_str."))) union all (select dkey as 'type',fileid,did from ".DB_TABLEPRE."distribution where isnull(viewdate) and uid='".$_USER->id."')");
	//$query = $db->query("SELECT a.type,a.fileid FROM ".DB_TABLEPRE."personnel a,".DB_TABLEPRE."personnel_log b,".DB_TABLEPRE."attachment c where (a.pkey=0 and a.name like '%".get_realname($_USER->id)."%' and a.appkey=2) or ((a.pkey=0 or a.pkey=4) and a.appkey=1 and a.id=b.perid and b.pkey=0 and b.uid=$_USER->id) and c.attakey=0 UNION SELECT a.type,a.fileid FROM ".DB_TABLEPRE."personnel a,".DB_TABLEPRE."personnel_log b,".DB_TABLEPRE."approval c where (a.pkey=0 and a.name like '%".get_realname($_USER->id)."%' and a.appkey=2) or ((a.pkey=0 or a.pkey=4) and a.appkey=1 and a.id=b.perid and b.pkey=0 and b.uid=$_USER->id) and c.akey=0 group by a.fileid");
	...
	while ($row = $db->fetch_array($query)) {
		...
		//$sql = "SELECT * FROM ".DB_TABLEPRE.$table."  WHERE id = '".$row['fileid']."'";
		$sql = "SELECT * FROM ".DB_TABLEPRE.$table."  WHERE ".$key."=0 and  id = '".$row['fileid']."'";
		$rs = $db->fetch_one_array($sql);
		if($rs['id']){
			echo '<ul id=OM_smart_'.$row['did'].'>';
			echo '<li>【';
		if($row['type']==1){
			echo '收文';
			if($rs['type']==1){
					echo'】【<font color="red">急件</font>';	
					}
			}else{
				echo '发文';
			}
		...
		echo ' <a href="admin.php?ac='.$table.'&do=personnel&fileurl=app&type=1&fileid='.$rs['id'].'&apptype='.$rs['category'].'" target=_blank>';
		echo '<font color="#FF0000">进入办理</font></a></li>';
		}else{
		
		echo '】 <a href="admin.php?ac='.$table.'&do=view&fileurl=app&fileid='.$rs['id'].'&apptype='.$rs['category'].'" target="_blank">'.$rs['title'].'</a>';
		echo ' <a onclick=OM_delete_db("'.$row['did'].'",""); href="admin.php?ac='.$table.'&do=reading&fileurl=app&fileid='.$rs['id'].'&apptype='.$rs['category'].'&did='.$row['did'].'" target=_blank>';
		echo '<font color="#0000FF">进入阅读</font></a> &nbsp; <font style="color:#f90;cursor:pointer" onclick=OM_delete_db("'.$row['did'].'","admin.php?ac='.$table.'&do=reading&fileurl=app&fileid='.$rs['id'].'&apptype='.$rs['category'].'&did='.$row['did'].'");>[已阅,不再显示]</font></li>';
		}
		echo '</ul>';
	}
	}
...
function OM_delete_db(did,url){
OM_ifr.document.location=url;


if (url==""){
setTimeout("document.getElementById('OM_smart_"+did+"').outerHTML='';",1000);

}else{

document.getElementById("OM_smart_"+did).outerHTML="";

}
return false;
}

</script><iframe id="OM_ifr" name="OM_ifr" style="display:none"></iframe>
<?	
	echo '</div>';
	echo '</div>';
	echo '</div>';
...
function home_duty($num){
	...
	echo '<div id="module_'.$num.'_body" class="module_body">';
	echo '<div id="module_'.$num.'_ul" class="module_div">';
	$query = $db->query("SELECT * FROM ".DB_TABLEPRE."project_duty WHERE uid='".$_USER->id."' or user='".get_realname($_USER->id)."' ORDER BY id desc LIMIT 8");
	while ($row = $db->fetch_array($query)) {
		echo '<ul>';
		echo '<li>【';
		if($row['dkey']=='1'){
		...
		echo '】<a href="admin.php?ac=duty_views&fileurl=duty&did=';
