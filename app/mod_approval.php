...
if ($do == 'list') {
	...
	if($_GET['type']==1){
		...
		$num = $db->result("SELECT count(*) as num FROM ".DB_TABLEPRE."distribution a,".DB_TABLEPRE."approval b WHERE a.uid='".$_USER->id."' AND a.dkey=2 AND a.fileid=b.id AND not ISNULL(a.viewdate)");
		$sql = "SELECT b.*,a.did FROM ".DB_TABLEPRE."distribution a,".DB_TABLEPRE."approval b WHERE a.uid='".$_USER->id."' AND a.dkey=2 AND a.fileid=b.id AND not ISNULL(a.viewdate) order by a.did asc LIMIT $offset, $pagesize";
	}elseif($_GET['type']==4){
		if ($title = getGP('title','G')) {
			$wheresql .= " AND a.title LIKE '%$title%' ";
			$url .= '&title='.rawurlencode($title);
...
//		$sql = "SELECT * FROM ".DB_TABLEPRE."approval WHERE 1 $wheresql  order by id desc LIMIT $offset, $pagesize";
		$num = $db->result("SELECT COUNT(*) AS num FROM ".DB_TABLEPRE."approval WHERE $wheresql order by id desc");
