...
if ($do == 'list') {
	...
	if($_GET['type']==1){
		...
		$sql = "SELECT b.* FROM ".DB_TABLEPRE."personnel a,".DB_TABLEPRE."attachment b WHERE b.id=a.fileid AND a.pkey IN(0,4) AND a.name LIKE '%".get_realname($_USER->id)."%' AND b.attakey<>1 AND a.type=1 AND (a.pkey=0 OR a.id IN($tmp_perid_str)) ORDER BY b.type=1 DESC,b.receiptdate DESC LIMIT $offset, $pagesize";
		...
		$num = $db->result("SELECT count(*) as num FROM ".DB_TABLEPRE."distribution a,".DB_TABLEPRE."attachment b WHERE a.uid='".$_USER->id."' AND a.dkey=1 AND a.fileid=b.id AND not ISNULL(a.viewdate)");
		$sql = "SELECT b.*,a.did FROM ".DB_TABLEPRE."distribution a,".DB_TABLEPRE."attachment b WHERE a.uid='".$_USER->id."' AND a.dkey=1 AND a.fileid=b.id AND not ISNULL(a.viewdate) order by a.did asc LIMIT $offset, $pagesize";
	}elseif($_GET['type']==4){
		if ($title = getGP('title','G')) {
			$wheresql .= " AND a.title LIKE '%$title%' ";
			$url .= '&title='.rawurlencode($title);
		...
		$num = $db->result("SELECT COUNT(*) AS num FROM ".DB_TABLEPRE."attachment a,".DB_TABLEPRE."personnel b WHERE 1 $wheresql and a.id=b.fileid and b.pkey!=0 and b.type=1 and b.name like '%".get_realname($_USER->id)."%'");
		...
		if ($title = getGP('title','G')) {
			$wheresql .= " title LIKE '%$title%' AND ";
			$url .= '&title='.rawurlencode($title);
		}
		if ($number = getGP('number','G')) {
			$wheresql .= " number='".$number."' AND ";
			$url .= '&number='.rawurlencode($number);
		}
		if ($organ = getGP('organ','G')) {
			$wheresql .= " organ LIKE '%$organ%' AND ";
			$url .= '&organ='.rawurlencode($organ);
		}
		if ($fontsize = getGP('fontsize','G')) {
			$wheresql .= " fontsize LIKE '%$fontsize%' AND ";
			$url .= '&fontsize='.rawurlencode($fontsize);
		}
		if ($category = getGP('category','G')) {
			$wheresql .= " category='".$category."' AND ";
			$url .= '&category='.rawurlencode($category);
		}
		//权限
		...
		//if($vuidtype==''){
			if ($_GET['type']!="")$wheresql .= "AND ";
			$wheresql .= "(uid='".$_USER->id."'";
		//}
		//if ($vuidtype!='') {
			//if($vuidtype=='-1'){
		...
		//}
//		$num = $db->result("SELECT COUNT(*) AS num FROM ".DB_TABLEPRE."attachment WHERE 1 $wheresql");
//		$sql = "SELECT * FROM ".DB_TABLEPRE."attachment WHERE 1 $wheresql  order by id desc LIMIT $offset, $pagesize";
		$num = $db->result("SELECT COUNT(*) AS num FROM ".DB_TABLEPRE."attachment WHERE $wheresql");
		$sql = "SELECT * FROM ".DB_TABLEPRE."attachment WHERE $wheresql  order by id desc LIMIT $offset, $pagesize";
	}
